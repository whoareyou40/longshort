...
class TestMomentumStrategy(MomentumStrategy):
    """Test version of the momentum strategy that doesn't make real trades"""
    ...
    def setup_exchange(self):
        ...
        self.logger.info("Using test data instead of real exchange")
    ...
    async def get_balance(self):
        ...
        self.logger.info(f"Mock positions: {self.asset_value}")
        
    async def cancel_all_orders(self):
        """Mock order cancellation"""
        self.logger.info("Mock: Cancelled all open orders")
        
    async def create_order(self):
        """Mock order creation - just log what would be done"""
        for trading_pair in self.config.TRADING_PAIRS:
            ...
            if current_status == 1 and current_value == 0:
                self.logger.info(f"TEST: Would open LONG position for {trading_pair}: {target_amount:.6f} @ ${current_price}")
                
            elif current_status == -1 and current_value == 0:
                self.logger.info(f"TEST: Would open SHORT position for {trading_pair}: {target_amount:.6f} @ ${current_price}")
                
            elif current_status == 0 and current_value > 0:
                self.logger.info(f"TEST: Would close LONG position for {trading_pair}")
                
            elif current_status == 0 and current_value < 0:
                self.logger.info(f"TEST: Would close SHORT position for {trading_pair}")

async def run_test():
    """Run a test of the strategy"""
    print("=" * 60)
    ...
    try:
        ...
        print("\n1. Testing momentum factor calculation...")
        await strategy.get_factor()
        
        print("\n2. Testing balance fetching...")
        await strategy.get_balance()
        
        print("\n3. Testing order creation logic...")
